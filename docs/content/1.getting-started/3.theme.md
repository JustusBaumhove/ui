...
#nuxt
:::tip
You can add your own dynamic color aliases in your `app.config.ts`, you just have to make sure to define them in the [`ui.theme.colors`](/getting-started/installation/nuxt#themecolors) option in your `nuxt.config.ts` file.
...
Nuxt UI automatically creates a CSS variable for each color alias you define which represents the default shade used in both light and dark modes:
...
::tip
You can change which shade is used for each color in light and dark mode:
